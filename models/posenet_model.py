...
class PoseNetModel(BaseModel):
    ...
    def get_current_errors(self):
        ...
        ori_gt = F.normalize(self.input_B[:, 3:], p=2, dim=1)
        #print(ori_gt, self.pred_B[1])
        #print(pos_err)

        #abs_distance = torch.abs((ori_gt.mul(self.pred_B[1])).sum())
        #ori_err = 2*180/numpy.pi* torch.acos(abs_distance) 

        abs_distance_A = (ori_gt.mul(self.pred_B[1])).sum()
        abs_distance_B = (-ori_gt.mul(self.pred_B[1])).sum()
        ori_err_A = 2*180/numpy.pi* torch.acos(abs_distance_A) 
        ori_err_B = 2*180/numpy.pi* torch.acos(abs_distance_B) 
        ori_err = ori_err_A.item() if ori_err_A.item() < ori_err_B.item() else ori_err_B.item()
        #rev_ori_err = 180 - ori_err if ori_err > 90 else ori_err
        #print(ori_err)

        return [pos_err.item(), ori_err]
